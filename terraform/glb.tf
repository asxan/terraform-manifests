...
resource "google_compute_global_forwarding_rule" "prod_lb_global_forwarding_rule" {
  depends_on = [
    google_compute_target_http_proxy.prod_lb_http_proxy,
  ...
  port_range            = "80"
  target                = google_compute_target_http_proxy.prod_lb_http_proxy.id
  ip_address            = google_compute_global_address.external_static_ip.id
}


# http proxy
resource "google_compute_target_http_proxy" "prod_lb_http_proxy" {
  depends_on = [
